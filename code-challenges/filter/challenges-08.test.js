...
const oddValues = (arr) => {
  let outValuesArray = arr.filter(num => (num % 2));
...
const filterStringsWithVowels = (arr) => {
  let vowels = arr.filter(string => string.match(/[a|e|i|o|u]/))
  return vowels;
};
...
Write a function named getBaseStatGreaterThan that, given the snorlaxData, below, and an inted eger as input, uses filter to return an array containing all stats with a baseStat greater than the integer.
...
const getBaseStatGreaterThan = (arr, minBaseStat) => {
  let statsFilter = arr.filter(stat => {
...
const getStatName = (arr, minBaseStat) => {
  ...
  statsFilter.forEach(obj => {
    statsName.push(obj.stat.name);
  });
  return statsName;
};
...
const getCharactersWithoutChildren = (arr) => {
  let charactersWithoutChildren = arr.filter(character => {
